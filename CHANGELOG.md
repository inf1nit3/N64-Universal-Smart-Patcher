...
- **Zelda - Ocarina of Time (USA) Rev 0: 640x480i hi-res developed in-project (EXPERIMENTAL).** With the zeldaret/oot decompilation of this exact revision as the map, the patch enables the game's own runtime-width machinery: ViMode 640x480i with the editor path active (gScreenWidth drives scissors, color-image strides and most 2D), the framebuffer pair relocated into the Expansion Pak area, and the 3D viewport at 640x480. Verified in mupen64plus: title and attract scenes render full-screen 640x480 with no corruption across 4800+ frames. Menus/HUD keep the original 320-space layout (upper-left quadrant) - the same known limitation the SubDrag patches ship with; a per-renderer 2D pass is future work. Shipped as a user-installable recipe + BPS in `scripts/oot_hires/` (not bundled: hardware validation pending). Stage 1 now also applies default-flavor BPS recipes, and recipe op files resolve against their recipe's own directory.
- Tests: 462 with every optional dependency installed, up from 410.
