...
- Tests: 493 with every optional dependency installed, up from 410. Five of them pin the shipped OoT recipe to a specific build: a BPS footer carries the CRC32 of its source and of its target, so a stale binary can no longer look identical to a current one.
