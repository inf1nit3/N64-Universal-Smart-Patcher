...
- **The SM64 menu fix ships.** `game_fixes/635A2BFF_sm64_menu_2x.ips` is the first entry in the per-game fix database: the four menu-glyph texture-rectangle emitters of Super Mario 64 (USA), each with its coordinate shift doubled (`x << 2` -> `x << 3`) and its texture step halved - 24 changed bytes, confirmed on a SummerCart64 (file-select labels and the Peach letter land at full size and position). Stage 1b applies it automatically on top of the SubDrag delta; the run log names it. The three HUD emitters stay excluded - with them enabled the HUD loses its numbers - and `scripts/sm64_hires/make_ips.py --bisect` produces installable bisect variants for the hardware run that decides which site is responsible. The generator needs no ROM: it decodes the hardware-verified `.wip` BPS (whose encoder emits every changed byte as a literal) and refuses to write unless it is exactly the site table's edit.
- **The GUI no longer freezes while it reads files.** The verified-hi-res check read a megabyte per ROM on the UI thread, again on every add; it now runs on a background worker whose results land in a session cache, with the checkbox honestly reading "checking" until the answer exists. Saves-tab inspect/convert moved to a worker as well (a dropped folder can hold hundreds of saves); a conversion that would overwrite an existing save is still decided on the UI thread, one question per file.
- **The GUI grows a menu bar and keyboard shortcuts.** Adding (Ctrl/Cmd+O, +Shift+O for folders, +Alt+O for saves), inspecting (Ctrl/Cmd+I), starting (Ctrl/Cmd+Return), cancelling (Esc) and tab switching (Ctrl/Cmd+1-4) drive the same slots as the buttons; the bar is styled to the console theme. Nothing is reachable only by mouse anymore.
- **The GUI can write its output elsewhere.** An output-folder picker on the Patching tab feeds `patch_rom`'s existing `output_dir` (persisted in QSettings; empty means next to each ROM, as before).
...
- **The whole verified recipe set surveyed for the 2D gap.** With the user-supplied clean dumps, `analyze.py` ran the same flow on every recipe: GoldenEye (33 packers), F-Zero X (169), Quake II (25) each leave their whole 2D layer in 320-space and carry prepared bisect variants; Forsaken shows a single site (likely another drawing idiom) and Golden Nugget's delta already covers its 2D layer. The shipped SM64 menu fix was validated end-to-end against the real dump: the local pipeline reproduces the hardware-tested base image byte-exactly and the IPS reproduces all 24 verified bytes. Banjo-Kazooie stays blocked - the available dump is v1.0, the recipe needs Rev A.
- **See which game fix will run — before running it.** `--list-game-fixes` prints every installed per-game fix with its source, marking a user-level fix that overrides a shipped one. The Inspector gains a **Game fix** column doing the same per loaded ROM (`635A2BFF_sm64_menu_2x.ips [user]` while a bisect file is installed) - and the DAT/Dump columns now actually carry their values, which a header-only test had let slip.
...
- **Zelda - Ocarina of Time (USA) Rev 0: 640x240p hi-res developed in-project (EXPERIMENTAL).** With the zeldaret/oot decompilation of this exact revision as the map, the patch drives the game's own static boot VI tables into a 640-wide progressive mode (VI_WIDTH 0x280, VI_X_SCALE 0x400 - NTSC 1.0 never re-issues `osViSetMode`, so those tables hold for the session), widens `gScreenWidth` so scissors and color-image strides follow, grows the framebuffer pair to 2x 640x240 with the 8MB fb-end in the Expansion Pak area, and opens View_Init's viewport so the 3D fills the width. gZBuffer is relocated out of the way, because a 640-wide scissor makes the RDP write 0x4B000 bytes of depth into a 0x25800-byte buffer. Verified in mupen64plus through logo -> title -> File Select -> gameplay. The interlaced 640x480i route was built first and **rejected**: interlace is unusable on the test setup. Menus/HUD keep the original 320-space layout - the same known limitation the SubDrag patches ship with; a per-renderer 2D pass is future work. Shipped as a user-installable recipe + BPS in `scripts/oot_hires/` (not bundled: hardware validation pending). Stage 1 now also applies default-flavor BPS recipes, and recipe op files resolve against their recipe's own directory.
- **SM64: the HUD problem is solved in the emulator.** The HUD's three texture-rectangle emitters step their texture by 4.0 per pixel where the menu emitters step by 1.0 - the signature of the RDP's COPY cycle type, which segment 2's HUD display list confirms (`G_CYC_COPY`). Copy mode cannot scale at all, so the menu fix's transform (double the shift, halve the step) was invalid there; that is why the HUD lost its numbers. `scripts/sm64_hires/make_hud.py` follows dataDave's SM64 H2X instead and moves the HUD to the 1-cycle pipeline: exclusive end coordinates, texture step 0.5, and both HUD display lists rewritten - combiner, point filter and render mode, with the words taken from H2X's compiled ROM. H2X adds two commands to each list, which cannot be done in place in a compressed segment without moving every later address; merging cycle type, perspective and filter into one `SETOTHERMODE_H` command frees exactly the two slots needed, and segment 2 recompresses to 48 392 bytes in its 48 400-byte slot. In mupen64plus the attract demos show the HUD at full size in its original layout - counters, camera icon and text alike. A `--mode copy` fallback keeps copy mode and doubles only positions. Both IPS files reproduce their test ROMs byte-for-byte through the real pipeline. Hardware run pending before anything replaces the shipped menu fix. This also corrects the 2026-09-16 note that blamed the `+15` end offset: in copy mode the end is inclusive, so `+15` is right.
- **hires2d: a guard against COPY mode, and an emulator pre-screen of every prepared variant.** `makefix2d.py` now refuses sites whose function carries a COPY-mode step word (dsdx 4.0 / dtdy 1.0) - the cause of SM64's broken HUD - unless told `--skip-copy-mode` or `--allow-copy-mode`; `analyze.py` reports the count. It separates SM64's three HUD emitters from its four menu emitters without a miss, finds none in GoldenEye, Quake II or Forsaken, and 46 of 169 sites in F-Zero X, whose variants are rebuilt without them. Running every variant through boot and attract mode in mupen64plus then showed most of them are not worth a hardware round: F-Zero X tears its title art (texture-space packers in the groups), Quake II smears its text and, with steps, distorts 3D geometry, and GoldenEye's step halving breaks intro credits the base image already renders correctly. Only GoldenEye A/C remain candidates for a console run.
- **Fixed: the shipped OoT recipe was four days stale.** `zelda-oot-usa-rev0-640x240p-exp.bps` still carried the 2026-09-12 build, without the gZBuffer relocation the builder had grown since - so installing the pair produced the ROM whose 640-wide scissor writes 0x4B000 bytes of depth into a 0x25800-byte buffer, over `gGfxSPTaskOutputBuffer`. Rebuilt from `640p --zrel`. Verified before shipping: the BPS round-trips byte-identically to the candidate, a full `--h2x` pipeline run reproduces it, and mupen64plus renders logo -> title -> gameplay with the whole picture present, the 2D layer in its 320-space corner, and clean depth. The candidate the BPS was nearly generated from turned out to be a *debug* build - the auto-advance edits sit in `ovl_file_choose`, past the first megabyte and therefore outside the CIC checksum window, so its CRC pair is indistinguishable from a clean build's.
- **Reverted: VI_X_SCALE 0x400 was right after all.** The 2026-09-13 reading - that the register is an upscale factor, so a 640-wide framebuffer needs `0x100` - is wrong, and the emulator shows it: with `0x100` the boot wordmark is cropped at the right edge and the title screen comes out magnified and cut off. Nintendo's own source settles the encoding. `viint.h` defines `SCALE(scaleup, off)` as `F210(1.0f / scaleup)`, so the register holds the *reciprocal*: the stock table's `SCALE(2, 0)` is 0.5 = `0x200` for 320 pixels. `z_vimode.c:175` states it as `xScale = (width << 10) / (SCREEN_WIDTH * 2 + rightAdjust - leftAdjust)` with `SCREEN_WIDTH` 320 - which gives `0x200` for 320-wide and `0x400` for 640-wide. `0x100` encodes a 160-wide framebuffer. That leaves the 2026-09-13 hardware failure unexplained by the VI tables; the candidate it tested predates `--zrel`, and a depth overflow fits "right and bottom garbage, unstable" far better than a scan-rate error.
- **Fixed: CI had been red since 2026-08-30.** The `lint-and-types` job failed on every one of the 44 commits since, and because `ruff check` runs first, neither `ruff format` nor `mypy` had run in that window. Nine ruff findings in the two RE builders (`makeoot.py`, `makemm.py`), two unformatted files behind them, and one `no-any-return` in `gui.py` that only appears without PyQt6 stubs - which is exactly how the lint job installs.
- **Fixed: the repository was missing objects.** Every `git log`/`git diff`/`git show` crossing 2026-09-10 aborted with an unreadable tree. Restored from origin with `git fetch --refetch`; `git fsck` now reports only unreferenced objects.
- **Built-in engines first, on every platform.** Checksums and verified deltas used to go through the bundled `rn64crc.exe` / `xdelta3.exe` whenever they could run - in practice only on Windows, so one platform took a different path from the other two. Both built-in engines are now checked against real data: the VCDIFF decoder matches the reference `xdelta3` byte-for-byte on all 9 bundled deltas whose dumps are available (Banjo-Kazooie Rev A and Pokemon Snap have none), and the CRC engine reproduces the header checksums of all 78 retail dumps in the collection. They now run first everywhere; `rn64crc` is only asked for a boot chip the engine does not know (and still only counts when the file verifies afterwards), `xdelta3` only for a delta the decoder cannot read. The CLI no longer warns about the two missing helpers on macOS/Linux - only a missing `u64aap` still changes a result.
- **Tooling brought up to date.** CI tests Python 3.14 as well (3.11-3.14), on `actions/checkout@v7` and `actions/setup-python@v7`. ruff and mypy are pinned in `requirements-lint.txt` (ruff 0.16.8, mypy 2.3.1) instead of taking whatever was released that morning, and `.github/dependabot.yml` proposes monthly grouped bumps for them and for the actions. The binary build now runs the full test suite, GUI tests included, before it freezes anything - until now a tag on a failing commit would have produced release binaries. `crunch64`, which `make_hud.py --mode full` needs, is part of the `dev` extra.
- Tests: 503 with every optional dependency installed, up from 410. Five of them pin the shipped OoT recipe to a specific build: a BPS footer carries the CRC32 of its source and of its target, so a stale binary can no longer look identical to a current one.
...
- **Fixed: emoji output could abort a run on Linux and macOS.** The UTF-8 stream reconfiguration was guarded by `sys.platform == "win32"`. A process started under the C/POSIX locale - the default in minimal containers and many CI images - gets an ASCII stdout, and the first emoji raised `UnicodeEncodeError` mid-batch. Now applied on every platform, to stdout and stderr, with `errors="replace"`.
- **Native binaries for macOS and Linux.** A new build workflow produces Windows x86_64, macOS arm64, macOS x86_64 and Linux x86_64 artifacts. The macOS GUI ships as a real `.app` bundle with a document type for `.z64`/`.n64`/`.v64`, archived with `ditto` so the bundle survives. The Linux tarball carries both binaries, a `.desktop` entry, an icon and `install-linux.sh`, which installs into `~/.local` without root and can uninstall itself.
- **New: `scripts/smoke_test.py`** drives the *installed* command line tool as a subprocess against real files - entry point, bundled recipe database, byte-order detection, patching, the hi-res gate, verification, manifest round trip, patch creation and re-application, and a parallel batch. 26 checks. It also accepts a path, so the frozen binary can be tested rather than the source tree. This is the class of check that caught two PyInstaller data-path bugs that every unit test passed.
- CI additionally runs that smoke test on Ubuntu, macOS and Windows against a built wheel, once more under `LC_ALL=C` with locale coercion disabled, and against every frozen binary at build time. The Linux GUI build is checked with `ldd` for unresolved shared libraries.
- Log files now follow each platform's convention: `%APPDATA%` on Windows, `~/Library/Logs` on macOS, and `$XDG_DATA_HOME` on Linux with the spec's fallback when it is unset or not absolute.
- Missing-helper warnings name the install command for the platform in use, and no longer describe `xdelta3` as degrading gracefully - it does not.
- New `tests/test_cross_platform.py` guards the assumptions that only break elsewhere: every `.xdelta` named in the patch database must resolve **case-exactly** (a mismatch is invisible on NTFS and turns every verified dump into "unsupported" on Linux), no two shipped assets may differ only by case, shipped filenames must be legal on Windows, no absolute Windows paths or `os.startfile` in the source, and the bundled `.exe` helpers must not carry an executable bit.
- PyInstaller specs are now a single platform-aware file per target rather than Windows-only: icon format per platform, UPX only where it is safe, and the macOS bundle. The data lists that broke twice before are declared once at the top of each spec.
- Tests: 281, up from 262.
