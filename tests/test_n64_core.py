...
class TestH2xFlavor(unittest.TestCase):
    """The 640x240 flavor route: BPS application, game-fix suppression,
    and refusing the generic widening when only 640x480 exists."""
    ...
    def setUp(self):
        ...
        self.tmp = tempfile.TemporaryDirectory()
        self.addCleanup(self.tmp.cleanup)
        self.clean = self._write("clean.z64", self._rom(self.SM64))
        # a real BPS built from this exact source: patch_rom's temp copy is
        # byte-identical to it, so the source CRC32 check passes for real
        target = bytearray(self._rom(self.SM64))
        target[0x40] ^= 0xFF
        self.target = bytes(target)
        self.bps = os.path.join(self.tmp.name, "h2x.bps")
        res = create_bps_patch(self.clean, self._write("target.z64", self.target), self.bps)
    ...
    def test_flavor_bps_applies_and_produces_the_target(self):
        with mock.patch.object(core, "get_flavor_patch", return_value=self.bps):
            result, log = self._run()
        self.assertEqual(result["status"], "patched", log)
        self.assertIn("HR", result["applied"])
        self.assertNotIn("GAMEFIX", result["applied"])
        with open(result["output"], "rb") as f:
            self.assertEqual(f.read(), self.target)

    def test_flavor_never_triggers_the_game_fix(self):
        """H2X reworks the 2D layer itself; a Stage 1b fix is built against
        the 640x480 image and must not touch the H2X output."""
        calls = []
        with (
            mock.patch.object(core, "get_flavor_patch", return_value=self.bps),
    ...
    def test_missing_flavor_refuses_the_generic_widening(self):
        """Asked for 640x240 on a dump that only has 640x480: refuse with
    ...
    def test_known_flavor_output_is_recognised_as_native(self):
        ...
        fake_outputs = {(0x65D3D6B2, 0x70B82FEB): "SM64 H2X 640x240 (dataDave)"}
        with mock.patch.object(core, "KNOWN_HIRES_OUTPUTS", fake_outputs):
            info = core.inspect_rom_details(rom)
        self.assertEqual(info["hires_support"], core.HIRES_NATIVE)
        self.assertIn("SM64 H2X", info["hires_support_reason"])
        # and re-patching it is a skip with an honest reason
        options = core.PatchOptions(hires=True, hires_flavor="640x240")
        lines = []
