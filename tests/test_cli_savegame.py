...
class TestSaveInfo(SaveCliTest):
    ...
    def test_a_missing_file_fails_cleanly(self):
        self.assertEqual(self._run("--save-info", os.path.join(self.tmp.name, "nope.sra")), 1)
...
class TestSaveConvert(SaveCliTest):
    ...
    def test_both_ends_must_be_named(self):
        ...
        self.assertEqual(self._run("--save-convert", self.src, "--save-to", "sc64"), 1)
        self.assertEqual(self._run("--save-convert", self.src, "--save-from", "mupen64plus"), 1)
    ...
    def test_default_name_uses_the_target_tools_extension(self):
        ...
        self.assertEqual(
            self._run(
                "--save-convert", self.src, "--save-from", "mupen64plus", "--save-to", "sc64"
            ),
            0,
        )
        expected = os.path.join(self.tmp.name, "THE LEGEND OF ZELDA-9EB1E8AC [sc64].sav")
        self.assertTrue(os.path.exists(expected), os.listdir(self.tmp.name))

    def test_an_unmeasured_tool_is_refused(self):
        self.assertEqual(
            self._run("--save-convert", self.src, "--save-from", "project64", "--save-to", "sc64"),
            1,
        )
    ...
    def test_a_file_of_no_known_chip_size_is_refused(self):
        ...
        self.assertEqual(
            self._run("--save-convert", odd, "--save-from", "mupen64plus", "--save-to", "sc64"), 1
        )
