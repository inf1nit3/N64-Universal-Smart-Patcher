...
def build_window(
    inst, data, addr, tgt_len, cpy_len=None, cpy_off=0, adler_target=None, delta_ind=0
):
    ...
    body = vint(tgt_len) + bytes([delta_ind]) + vint(len(data)) + vint(len(inst)) + vint(len(addr))
    if adler_target is not None:
...
class TestVarint(unittest.TestCase):
    def test_roundtrip(self):
        for n in (0, 1, 127, 128, 255, 300, 0x3FFF, 0x4000, 0x800000, 0xFFFFFFF, 12345678901):
...
class TestHandcraftedVectors(unittest.TestCase):
    def test_add_only_window_without_source(self):
        win = build_window(
            bytes([op_add_immediate(5)]), b"hello", b"", tgt_len=5, adler_target=b"hello"
        )
    ...
    def test_add_explicit_large_size(self):
        ...
        win = build_window(inst, payload, b"", tgt_len=300, adler_target=payload)
        self.assertEqual(xp.decode_xdelta(build_patch([win]), b""), payload)

    def test_run_instruction(self):
        win = build_window(
            bytes([OP_RUN_EXPLICIT]) + vint(7), b"\x42", b"", tgt_len=7, adler_target=b"\x42" * 7
        )
    ...
    def test_copy_self_here_near_same_caches(self):
        ...
        self.assertEqual(out, source[10:16] + source[20:24] + source[13:18] + source[10:16])
    ...
    def test_double_instruction_pairs(self):
        ...
        win = build_window(bytes([OP_COPY0_4_ADD1]), b"Y", vint(50), tgt_len=5, cpy_len=256)
        out = xp.decode_xdelta(build_patch([win]), source)
        self.assertEqual(out, source[50:54] + b"Y")

    def test_multiple_windows_and_cache_reset(self):
        w1 = build_window(bytes([op_add_immediate(3)]), b"abc", b"", tgt_len=3, adler_target=b"abc")
        w2 = build_window(bytes([op_add_immediate(2)]), b"de", b"", tgt_len=2, adler_target=b"de")
        self.assertEqual(xp.decode_xdelta(build_patch([w1, w2]), b""), b"abcde")

    def test_application_header_skipped(self):
        win = build_window(bytes([op_add_immediate(2)]), b"ok", b"", tgt_len=2, adler_target=b"ok")
    ...
    def test_window_without_source_flag(self):
        ...
        win = build_window(
            bytes([op_add_immediate(4)]), b"data", b"", tgt_len=4, adler_target=b"data"
        )
    ...
    def test_overlapping_target_copy(self):
        ...
        self.assertEqual(xp.decode_xdelta(build_patch([win]), b""), b"ab" * 4)
    ...
    def test_secondary_compression_rejected_header(self):
        with self.assertRaisesRegex(xp.XdeltaPatchError, "secondary compression"):
            xp.decode_xdelta(xp.VCDIFF_MAGIC + b"\x01", b"")

    def test_secondary_compression_rejected_window(self):
        win = build_window(bytes([op_add_immediate(1)]), b"x", b"", tgt_len=1, delta_ind=1)
        with self.assertRaisesRegex(xp.XdeltaPatchError, "delta indicator"):
    ...
    def test_adler_mismatch_detected(self):
        win = build_window(
            bytes([op_add_immediate(5)]), b"hello", b"", tgt_len=5, adler_target=b"WRONG"
        )
    ...
    def test_source_too_short(self):
        source = b"tiny"
        win = build_window(bytes([op_copy(0, 4)]), b"", vint(0), tgt_len=4, cpy_len=10000)
    ...
    def test_copy_address_out_of_range(self):
        ...
        win = build_window(bytes([op_copy(0, 4)]), b"", vint(100), tgt_len=4, cpy_len=10)
        with self.assertRaisesRegex(xp.XdeltaPatchError, "copy address out of range"):
            xp.decode_xdelta(build_patch([win]), b"0123456789")

    def test_truncated_sections(self):
        good = build_window(bytes([OP_ADD_EXPLICIT]) + vint(100), b"x" * 100, b"", tgt_len=100)
        with self.assertRaises(xp.XdeltaPatchError):
...
@unittest.skipUnless(XDELTA3_BIN, "xdelta3 binary not available on this machine")
...
class TestBundledSubDragPatches(unittest.TestCase):
    ...
    def _patches(self):
        ...
        return [f for f in sorted(os.listdir(PATCH_DIR)) if f.lower().endswith(".xdelta")]
    ...
    def test_sm64_patch_shape(self):
        name = next(n for n in self._patches() if n.startswith("Super Mario 64"))
        with open(os.path.join(PATCH_DIR, name), "rb") as f:
