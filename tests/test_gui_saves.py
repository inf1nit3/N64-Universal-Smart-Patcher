...
class TestSaveTab(unittest.TestCase):
    def setUp(self):
        ...
        self.src = self._write("THE LEGEND OF ZELDA-9EB1E8AC.sra", sg.swap_words(oot_chip_order()))
    ...
    def _select(self, source, target):
        self.win.save_from.setCurrentIndex(self.win.save_from.findData(source))
        self.win.save_to.setCurrentIndex(self.win.save_to.findData(target))

    def test_the_tab_offers_every_measured_tool(self):
        keys = {self.win.save_from.itemData(i) for i in range(self.win.save_from.count())}
    ...
    def test_convert_writes_the_flashcart_arrangement(self):
        ...
        gui.QFileDialog.getExistingDirectory = staticmethod(lambda *a, **k: out_dir)
        ...
        self.assertEqual(self._read(os.path.join(out_dir, written[0])), oot_chip_order())
    ...
    def test_an_existing_save_is_kept_when_the_user_declines(self):
        ...
        out_dir = os.path.join(self.tmp.name, "out")
        os.makedirs(out_dir)
        existing = os.path.join(out_dir, "THE LEGEND OF ZELDA-9EB1E8AC [sc64].sav")
        ...
        self._select("mupen64plus", "sc64")
        gui.QFileDialog.getExistingDirectory = staticmethod(lambda *a, **k: out_dir)
        gui.QMessageBox.question = staticmethod(lambda *a, **k: QMessageBox.StandardButton.No)

        self.win.convert_saves()
        drain(self.win)

        self.assertEqual(self._read(existing), b"someone's actual progress")
        self.assertIn("kept the existing file", self.win.save_output.toPlainText())
    ...
    def test_it_is_replaced_only_when_the_user_agrees(self):
        ...
        self._select("mupen64plus", "sc64")
        gui.QFileDialog.getExistingDirectory = staticmethod(lambda *a, **k: out_dir)
        gui.QMessageBox.question = staticmethod(lambda *a, **k: QMessageBox.StandardButton.Yes)
