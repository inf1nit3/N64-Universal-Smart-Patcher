...
**Zelda - Ocarina of Time (USA) Rev 0: an experimental 640x240p build is
...
boot VI tables are driven into a 640-wide progressive mode, `gScreenWidth`
is widened so scissors and strides follow, the framebuffer pair and the
z-buffer move into the Expansion Pak area, and the viewport opens so the
3D world renders full-width. Progressive, not interlaced — the 640x480i
route was built first and rejected on hardware as unusable. Menus and HUD
keep the original 320-space layout (upper-left quadrant) for now. Requires
an Expansion Pak. Emulator-verified; hardware validation pending —
therefore not bundled with the tool by default.
